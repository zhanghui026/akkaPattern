...
class ShardedPipeline(redis:ShardedJedisPipeline) extends Pipeline {
  ...
  def hincrBy(key: String, field: String, increment: Long) = redis.hincrBy(key, field, increment.intValue())
  ...
  def lrange(key: String, start: Long, end: Long) = redis.lrange(key, start.intValue(), end.intValue())
  
  def sort(key: String, sp: SortingParams) = redis.sort(key, sp)
  
  def rpush[T](key: String, value: T) = redis.rpush(key, value.toString)
  
  def lpush[T](key: String, value: T) = redis.lpush(key, value.toString)
  
  def ltrim(key: String, start: Long, end: Long) = redis.ltrim(key, start.intValue(), end.intValue())
